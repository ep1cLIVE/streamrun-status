from flask import Flask, request, jsonify, render_string_as_template
import os
import requests
    ...
    <body>
        ...
        <script>
            ...
            function updateStatus() {
                fetch(`${API_BASE}/streamrun/status`)
                    .then(r => r.text())
                    .then(text => {
                        document.getElementById('instanceInfo').innerHTML = text;
                        refreshInstanceData();
                    })
            ...
            function goLive() {
                callAPI('/streamrun/golive');
            }
            
            function stopInstance() {
                if (confirm('Stop stream instance?')) {
                    callAPI('/streamrun/stop');
                }
            }
            
            function toggleLive() {
                callAPI('/streamrun/outputs?state=LIVE');
            }
            
            function toggleOffline() {
                callAPI('/streamrun/outputs?state=OFFLINE');
            }
            ...
            function switchIngest() {
                ...
                callAPI(`/streamrun/switch_ingest?destination_id=${encodeURIComponent(destId)}`);
            }
    ...
    if instances_r.ok:
        ...
        if instances:
            ...
            if instance_id:
                ...
                return f"Starting stream with instance {instance_id}. Takes 10-30 seconds to go RUNNING.", 200

    return "Starting stream. Please run !status to confirm.", 200
